struct Rectangle{VT,PT,V<:AbstractVector{VT}}
  ...
  Rectangle(min, max, vertex_data::Union{AbstractVector,Nothing}, primitive_data) = Rectangle(Box(min, max), vertex_data, primitive_data)
  Rectangle(semidiag::Point, vertex_data::Union{AbstractVector,Nothing}, primitive_data) = Rectangle(Box(semidiag), vertex_data, primitive_data)
end
